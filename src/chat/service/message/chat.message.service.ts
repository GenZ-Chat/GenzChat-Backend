...
import { MessagesByChatDto, MessagesArrayDto } from "src/chat/dto/dto.messages_response";
...
export class ChatMessageService {
    ...
    async getMessagesByChatId(chatId: string): Promise<MessagesArrayDto> {
        ...
        const messages = await this.messageModel.find({ chat: new Types.ObjectId(chatId) }).sort({ createdAt: 1 }).exec();
    ...
    async getAllMessagesByUserId(userId: string): Promise<MessagesByChatDto> {
        ...
        const messages = await this.messageModel.find({ chat: { $in: chatIds } }).sort({ createdAt: 1 }).exec();

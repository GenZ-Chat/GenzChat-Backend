...
export class ChatService {
  ...
  async getChats(userId: string) {
    ...
    if (!Types.ObjectId.isValid(userId)) {
      throw new Error(`Invalid ObjectId for userId: ${userId}`);
    }
    ...
    const enrichedChats = await Promise.all(
      filteredChats.map(async (chat) => {
        ...
        const userWithStatus = await Promise.all(
          chatObject.users.map(async (user: any) => {
    ...
    const chatDTO = plainToInstance(ChatDto, enrichedChats, {
  ...
  async getGroupChatInfoByUserId(userId: string) {
    ...
    const groupChats = await this.chatModel
      .find({ type: ChatType.GROUP, users: userId })
      .populate('users')
      .exec();
    return groupChats;
  }
